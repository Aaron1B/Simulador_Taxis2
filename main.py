...
def ejecutar_simulacion():
    ...
    for t in all_threads:
        t.daemon = True # Cierre automático con main
        t.start()

    for dia in range(1, num_dias + 1):
        print(f"\n📅 === INICIO DÍA {dia} ===")
        for hora in range(6, 25): # Ciclo 06:00 a 24:00
            sistema.actualizar_tiempo(dia, hora)
            print(f"\n⌚ --- {hora}:00 H ---")
            time.sleep(1.5) 

        print("⏸️  Pausando actividad para registrar logs finales...")
        time.sleep(1.5) # Calma para logs finales
        
        print(f"🌙 FIN DEL DÍA {dia}: Realizando recuento y balance...")
        sistema.balance_final_dia() # Recuento al final del día
